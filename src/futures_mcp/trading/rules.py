...
``bars`` are the window's bar records (``time_chart_utc5``, ``day_chart``,
``high``, ``low``, ``close``), ``structure`` is one entry of
``pipeline.analyze(...)["structures"]`` (``S``, ``R``, ``verdict``, ``broke``
and ``rejections`` with ``n``/``side``/``idx``/``price``/``time``), and
``account`` carries ``equity``, ``risk_pct``, ``point_value``,
``min_contracts`` and the target ``targets`` R multiples.

The returned dict is either ``{"signal": False, "reason": ...}`` or a plan with
``direction``, ``order``, ``entry``, ``stop``, ``risk_points``, ``contracts``,
``targets`` and the exit instructions. Every plan is re-checked by
:func:`validate` before it leaves the server, so a rules bug cannot hand the
caller a stop on the wrong side of the entry.
"""
